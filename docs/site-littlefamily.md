...
## 1.5 検査する仕様は 3 つだけ (2026-08-21 運用側の確認)
...
| # | 対象 | 仕様 |
|---|---|---|
| 1 | カカクコム (`littlefamily03*`) | 通常 LP から**専用 LP (`/lp/service-premium/`) へリダイレクトする** |
| 2 | みらやく ○ | 通常 LP のまま。**ヘッダーに代理店名**、**フッターに「募集代理店：<会社名>」** |
| 3 | みらやく × | 2 と同じ表示に加えて、**LP 内に「あんしんパック」の記載が一切ない** |
| 4 | 全代理店 | **申込フォームに遷移してもコードが維持される** |

### 4. 申込フォームでのコード維持 (方式を問わず検査する)

引き継ぎ方式 (クエリ / hidden / Cookie / セッション / API) は未確定のままだが、
**「維持されているか」は方式を知らなくても検査できる**。

申込ボタン (`selectors.ctaPrimary`) を押して申込ドメインへ遷移し、
コードの値が次のどこかにあるかを見る。1 つでも見つかれば維持されている。

- URL (クエリ・パス)
- 入力値 (hidden を含む)
- 画面に表示されているテキスト
- `data-agency-code` などの data 属性
- localStorage / sessionStorage
- Cookie
- `dataLayer` などの JavaScript 変数

| 結果 | 重大度 |
|---|---|
| どこにも無い | **Critical** (申込がこの代理店に帰属しない) |
| 別の代理店コードが現れた | **Critical** (誤帰属) |
| 申込フォームへ遷移できない | **Critical** (申込導線の停止) |
| 見つかった | Low (どこにあったかを記録) |

> **「項目があるだけ」を合格にしない。**
> `agency_code` という hidden 項目があっても値が空なら維持されていない。
> 値が実際に見つかった場合だけ合格とし、項目の存在は参考情報として出す。

押すのは申込ボタンだけで、**入力も送信も行わない**。
本番は読み取り専用のため、GET 以外は実行前に遮断される。
