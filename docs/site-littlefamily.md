...
| 支店コード (末尾 `brNN`) | **通常 LP のまま何も変化しない**。カカクコムの支店コードもリダイレクトしない | 新パターン `branch-code` (151 件) |
| `littlefamily99` | みらやく ○ | マスタを ○ に修正 |

支店コードは 214 件のうち **151 件**を占める。
親コードの分類 (みらやく可否・カカクコム) より前に判定するため、
`assign` の順序が重要 (`codeMatches: "br[0-9]+$"` を先に置く)。

`branch-code` は `recognized: false` にしている。
サイト側でコードとして扱われないため、次を期待しない。

- Cookie / localStorage への保存
- 申込側への引き継ぎ
- 募集代理店名などの表示

また `sameAsNoCodeProfiles` に入れているため、
**コードなしの表示と一致すること**を検査する。
支店コードが有効になった (仕様変更) 場合も、
意図せず表示が変わった場合も、どちらも Medium として報告される
(どちらか断定できないため CI は失敗させない)。

> 未解決: 有効なコードで入った後に支店コードで再流入すると、
> 保存済みの前のコードが残る (サイトが支店コードを無視するため)。
> 実サイトは `storage.type: none` のため現在は検査していないが、
> 帰属先が前の代理店のままになる点は仕様確認が必要。
