...
| `selfBlockedPatterns` | このツールの安全装置による遮断。まとめて 1 件の Low にする |
| `thirdPartyScriptSeverity` | 他社タグのスクリプト内部で起きたエラーの重大度 (既定 `low`) |
| `maxDistinctMessages` | 同じ内容をまとめたうえで記録する種類の上限 (既定 50) |
...
### 自作自演を報告しない (安全装置による遮断)

本番など読み取り専用の環境では、このツールが GET 以外のリクエスト
(計測タグの送信など) を実行前に止めている。
ブラウザはそれを `net::ERR_BLOCKED_BY_CLIENT` として
コンソールに出すため、そのまま報告すると
**自分が止めたものを不具合として数えてしまう**
(実測で 1 回の実行に 3860 件)。

`selfBlockedPatterns` に一致するものは不具合として扱わず、
**まとめて 1 件の Low** として記録する
(「送信リクエストを遮断しました (サイトの不具合ではありません)」)。

### 他社タグのエラーは Critical / High にしない

計測・解析・広告タグのスクリプト内部で起きたエラーは自社コードの不具合ではない。
エラーの発生元 URL が検査対象と別オリジンなら、
`thirdPartyScriptSeverity` (既定 Low) で記録する。
無視はしない (表示を壊すことがあるため)。

自社ドメインのスクリプトのエラーは従来どおり High。

### 同じ内容のエラーはまとめる

同一メッセージが数千件出るサイトがある。1 件ずつ Finding にすると
実行が這うように遅くなり、レポートも読めなくなる。
同じ内容 (種別・本文・発生位置) は 1 件にまとめて件数を数え、
記録する種類の数は `maxDistinctMessages` (既定 50) で打ち切る。
...
## 7.2.1 「保存先」の列 (`observeStorageLocation` / `utils/agency.ts`)

Cookie・localStorage・sessionStorage を全部見て、代理店コードの文字列が
入っている場所を記録する。キー名を知らなくても分かるように値で探す。
正解 (どこに保存するのが正しいか) は未確定なので合否は判定しない。

| 表示 | 意味 |
|---|---|
| `Cookieのみ` | Cookie にだけあった |
| `Cookie+localStorage (両方)` | **両方にあった** |
| `なし` | どこにも無かった |
| `... (他社タグに N 件)` | 上記に加えて、他社タグの保存の中にも N 件あった |
| `自社の保存なし (他社タグに N 件)` | 自社の保存には無く、他社タグの中だけにあった |

**他社タグの保存を自社の保存と混ぜない。**
Zoho PageSense (`zps-` / `zab_`) や GA (`_ga`) は訪問した URL を保存するため、
URL に代理店コードが入っていると「サイトがコードを保持している」ように見える。
本番でこれを保存先として並べていたため、`config/agency.yml` の
`storage.thirdPartyKeyPatterns` に一致するキーは別に数える。

区切りに ` / ` を使わない。「または」と読めるため、
「両方にあった」を「どちらかにあった」と読み違える (実際に起きた)。

`storage.type` も同じ理由で書き分ける。
...
|---|---|---|
| HTTP 3xx | ○ | |
| JavaScript による遷移 | ○ | 追加のドキュメント要求として現れる |
| meta refresh | ○ | |
| SPA (history) | ○ | パスが変わる場合のみ |
| **同じ URL の再読み込み** | **×** | 遷移ではない。Low として記録だけ残す |

再読み込みは「続けて同じ URL を読み込んだ」ものだけを対象にする。
離れた位置での重複は「戻ってきた」= ループの可能性があるため別に扱う。

リダイレクト直後に再読み込みするサイトで
「リダイレクト回数が仕様と異なります」(High) を誤報したため、
数えないようにしたうえで `[記録] 同じページが再読み込みされています` を
Low で残している (数えないだけで消すと、挙動が変わったときに気づけない)。

## 7.3 検査開始時の状態 (`startClean` / `utils/agency-entry.ts`)

代理店コードは Cookie / localStorage に保存され、次に開いたページを
その代理店として表示する。**前の検査の状態が残っていると不具合を隠す。**

| 残留があると | 実際 | 判定 |
|---|---|---|
| コードが付与されていない | 前回のコードが残っている | 「付与されている」✅ (誤った合格) |
| 代理店名が出ない | 前回のコードで復元される | 「出ている」✅ (誤った合格) |
| コードなしで代理店名が出てはいけない | 前回のコードが残っている | 検知されるべき不具合を見逃す |

検知が無いことを合格の根拠にしていないチェックリストでも、
**残留は「検知が出ない」側に働く**ため危険度が高い。

そのため各テストの**最初の流入時**に次を行う。

| 手順 | 内容 |
|---|---|
| 1. 確認 | `context.cookies()` と `context.storageState().origins` を読む |
| 2. 報告 | 空でなければ **Critical** (`config`)。「検査環境の問題」と明記し、サイトの不具合と区別する |
| 3. 消去 | Cookie をコンテキストごと消し、開けていれば localStorage / sessionStorage も消す |

| 判断 | 理由 |
|---|---|
| 消すだけにしない | 黙って消すと、汚れた状態で走り始めていたことに誰も気づけない |
| 2 回目以降は何もしない | 「別コードで再流入」「コード保持後の再訪リダイレクト」は前の状態が必要な検査 |
| Critical にする | 残留があった実行は、他の ✅ も信頼できない |

Playwright はテストごとに新しいコンテキストを作るため通常は既に空で、
実測でも Cookie・localStorage・代理店表示のいずれも次のテストへ持ち越さない。
それでも毎回確認するのは、将来 `storageState` (ベーシック認証の使い回しなど) を
入れたときに黙って汚れた状態で走り始めるのを防ぐため。
...
| 空白画面 | `blank.html` | 可視テキスト不足が High として検出される |
| 極端に大きな要素 | `huge.html` | viewport 高さの倍数超過が検出される |
| URL の不要パラメータ | — | 許可外パラメータが Medium として検出される |
| リンク検査の上限 | `/lp/` | `maxLinksPerPage` が効く |
| ページ間の表記揺れ | — | ページごとに表記が分かれている場合に検出される |
