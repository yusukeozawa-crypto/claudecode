  ...
  const known = {
    'code-effective': {
      ...
      pass:
        ...
        + '「変化なし」= コードなしと完全に一致 (右の項目を読む意味がない)。'
        + '「変化なしの疑い」= 差分はあるが期待した変化が 1 つも起きていない (人が目で確認する)。'
        + '「変化あり」= 期待した変化が起きている (右の項目を個別に読む)。'
        + '自社コードなど「変わらないのが正しい」代理店は対象外。',
      severity: '変化なし: Critical / 変化なしの疑い: High (要目視) / 対象外: 記録のみ',
...
function limitsTab(ctx) {
  ...
  items.push({
    title: '「① コードの反映」は最終判断ではありません',
    detail:
      '出せるのは「コードを付けたら表示が変わったか」という実測だけです。'
      + '変わらなかった理由 (コードが未登録 / 適用が壊れている / 運用側が意図的に止めている) は'
      + '区別できません。実際に、保留中で止めていた代理店も「変化なし」と出ました。'
      + '白黒を付けるには、その代理店コードの登録状況をサイト側で確認する必要があります。'
      + 'また A/B テストの割り当てだけでも差分は出るため、差分があることは'
      + '「反映された」証拠になりません (そのため「変化なしの疑い」を別に出しています)。',
  });
  items.push({
