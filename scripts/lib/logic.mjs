...
function checkRows(root, ctx) {
  ...
  const known = {
    'code-effective': {
      observe:
        'コードを付けずに開いたページと、コードを付けて開いたページで、'
        + '「表示ブロック」と「文言」を丸ごと比べる。'
        + 'あわせて、期待した変化 (代理店名が出る / 安心パックが消える) が'
        + '実際に起きたかをその場で測る。',
      pass:
        '3 段階で出す。総合判定ではなく、最初に見る 1 歩目。'
        + '「不発」= コードなしと完全に一致 (右の項目を読む意味がない)。'
        + '「不発の疑い」= 差分はあるが期待した変化が 1 つも起きていない (人が目で確認する)。'
        + '「発火」= 期待した変化が起きている (右の項目を個別に読む)。'
        + '自社コードなど「変わらないのが正しい」代理店は対象外。',
      severity: '不発: Critical / 不発の疑い: High (要目視) / 対象外: 記録のみ',
  ...
  rows.push([
    '文言',
    '表示テキストを抽出し、社内規定「LF表記・ブランドルール」(2025/12/11 版) と'
      + '照合する。あわせて誤字候補・使用禁止表現も見る。',
    '規定の「正しい表記」になっていること。使用禁止表現がないこと。',
    'Low / ブランド・倫理と使用禁止表現は Medium (必ず報告する)',
...
function limitsTab(ctx) {
  ...
  items.push({
    title: '「① コードで発火」は最終判断ではありません',
    detail:
      '出せるのは「コードを付けたら表示が変わったか」という実測だけです。'
      + '不発だった理由 (コードが未登録 / 適用が壊れている / 運用側が意図的に止めている) は'
      + '区別できません。実際に、保留中で止めていた代理店も「不発」と出ました。'
      + '白黒を付けるには、その代理店コードの登録状況をサイト側で確認する必要があります。'
      + 'また A/B テストの割り当てだけでも差分は出るため、差分があることは'
      + '「発火した」証拠になりません (そのため「不発の疑い」を別に出しています)。',
  });
  items.push({
  ...
  items.push({
    title: '文言の基準は社内規定ですが、正式名称はまだ本番のものではありません',
    detail:
      '表記の判定は社内規定「LF表記・ブランドルール」(2025/12/11 版) を'
      + 'config/text-rules.yml に写したものです。'
      + 'ただし正式名称 (canonical の会社名・商品名) はモックサイトの名称のままで、'
      + '本番の会社名・普通約款名に差し替えていません。'
      + 'そのため「正式名称の誤表記」の検査は本番では動いていません。'
      + 'また判定は文字列の一致で行うため、文脈の判断 (「時」が形式名詞かどうかなど) は'
      + '限られた書き方しか見ていません。'
      + '募集文書チェックシートの 31 項目は、この検査には入っていません。',
  });
  items.push({
