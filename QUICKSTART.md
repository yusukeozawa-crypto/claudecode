...
`run-qa.cmd` を**ダブルクリック**すると、ブラウザに操作画面が開きます。
黒い画面（コマンド）を触る必要はありません。

画面でできること:

| | |
|---|---|
| 検査する | 「練習用サイト」「ステージング」「本番」のボタンを押すだけ |
| 進行状況 | 「いま確認中: 代理店の表示 (23/94)」のように日本語で表示 |
| 結果 | **異常なし / 問題あり** を最初に大きく表示 |
| 代理店ごとの結果 | 今回検査したコードごとに ○×（表示・リダイレクト・引き継ぎ…） |
| 対象サイトの設定 | URL とベーシック認証をフォームで登録（`.env` に保存） |
| 更新 | 「最新版に更新」ボタン |
| 履歴 | これまでの実行結果の一覧 |

黒い画面（`run-qa.cmd` のウィンドウ）は**開いたままにしてください**。
閉じると操作画面も止まります。

必要なのは Node.js だけです（https://nodejs.org/ja）。
初回は部品のダウンロードで 5〜10 分かかります。

macOS / Linux では `npm run ui` が同じ入口です。
コマンドで完結させたい場合は `npm run qa`（番号を選ぶ従来の画面）も使えます。
