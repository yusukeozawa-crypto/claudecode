...
|---|---|---|---|
| `self-test` | 同梱モックサイト (`QA_ENV=local`) | 不要 | 全ブランチへの push / PR / 手動 / 定時 |
| `qa` | ステージング / 本番 | 必要 | main への push / 手動 / 定時 |
...
3. **Pull Request / 作業ブランチへの push** — `self-test` のみ
   (実サイトを対象にする `qa` ジョブは main・手動・定時のみ)
